    ...
    if request.body_exists:
        ...
        try:
            if get_invoice_status(data) == 'unpaid':
                logging.info(
                    date_now + " [create payment] " + get_order_id(data) + ' ' + get_invoice_status(data) + ' ' + str(
                        get_invoice_amount(data)))
                ...
                #    payed(con,id)
                # con.close()
                logging.info(date_now + " [confirming payment] " + get_order_id(data) + ' ' + get_invoice_status(
                    data) + ' ' + str(get_invoice_amount(data)))
            elif get_invoice_status(data) == 'cancelled':
                logging.info(date_now + " [cancelled payment] " + get_order_id(data) + ' ' + get_invoice_status(
                    data) + ' ' + str(get_invoice_amount(data)))
            else:
                pass
        except:
